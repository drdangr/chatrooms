...
2. **Supabase миграции** должны быть применены:
   ...
   - `017_allow_update_files_openai_file_id.sql`
   - `018_setup_file_storage.sql`
   - `019_remove_mock_files.sql`
   - `020_add_assistant_summary.sql`
...
### Тесткейс 9: Саммари ассистента и гибридный режим

**Цель:** Убедиться, что резюме ассистента сохраняется в базе и используется при переключении между чат-режимами.

**Шаги:**
1. В комнате без файлов отправьте 2–3 сообщения, получите ответ от модели (обычный чат).
2. Загрузите файл, дождитесь автоматического включения Assistants API и задайте вопрос с ссылкой на файл.
3. Удалите файл, вернитесь в обычный чат и попросите модель пересказать предыдущий разговор.

**Ожидаемый результат:**
- В обычном чате модель опирается на краткое резюме предыдущего общения (хранится в `rooms.assistant_summary`).
- При наличии файлов ассистент продолжает разговор без потери контекста.

**Дополнительно:**
- В Supabase можно проверить поле `assistant_summary` в таблице `rooms`.

### Тесткейс 10: Контекстное меню сообщений

**Цель:** Проверить новые действия в меню «…» у сообщений.

**Шаги:**
1. Наведите курсор на сообщение и нажмите «…» (у ответов пользователя меню слева, у ассистента — справа).
2. Выберите «Выделить» — сообщение попадает в набор выбранных.
3. Нажмите «Саммаризировать» для одного или нескольких сообщений — появляется системное сообщение с конспектом.
4. Нажмите «Сохранить в файл» — скачивается `.md` с выбранными сообщениями, отображаются авторы и оригинальная разметка.
5. Нажмите «Удалить» — удаляются все выбранные сообщения (нужны права writer или выше).

**Ожидаемый результат:**
- Меню работает для каждого сообщения, элементы блокируются во время выполнения действий.
- Саммари и экспорт учитывают выделенные сообщения.
- После удаления сообщения исчезают из чата и базы.
