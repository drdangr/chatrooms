...
- 🤖 **Гибридный режим LLM** — приложение автоматически переключается между обычным чатом и Assistants API, подхватывая историю и файлы комнаты
- 🧠 **Контекстные саммари** — ассистент сохраняет краткое резюме беседы, поэтому при возврате в чат контекст не теряется
- 🗂️ **Файлы в диалоге** — загружайте документы и изображения, ассистент видит их через OpenAI Assistants API
- 🔍 **Семантический поиск** — ищите сообщения по смыслу, а не по точному совпадению слов
- 🛠️ **Контекстное меню сообщений** — выделяйте, саммаризируйте, скачивайте в Markdown и удаляйте сообщения прямо из чата
...
3. Примените миграции по порядку:
   ...
   - `013_add_room_temperature.sql` - поддержка температуры в комнатах
   - `014_add_assistants_support.sql` - таблица привязки Assistants API
   - `015_create_test_room_with_mock_files.sql` - тестовая комната и мок-файлы
   - `017_allow_update_files_openai_file_id.sql` - обновление `openai_file_id`
   - `018_setup_file_storage.sql` - конфигурация Supabase Storage
   - `019_remove_mock_files.sql` - очистка мок-файлов после тестов
   - `020_add_assistant_summary.sql` - хранение саммари ассистента
